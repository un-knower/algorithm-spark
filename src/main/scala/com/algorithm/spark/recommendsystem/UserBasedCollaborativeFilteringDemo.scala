...
/**
  * author: tangjun
  * time: 2017/10/20 17:30
  * description: 实现基于用户的协同过滤算法
  * 核心思想：计算目标用户的相似用户，并根据相似用户对物品的打分来预测目标用户对指定物品的评分
  */
object UserBasedCollaborativeFilteringDemo {

  def main(args: Array[String]): Unit = {
    System.setProperty("hadoop.home.dir", "D:\\software2\\hadoop-2.7.4")
    val conf = new SparkConf()
      .setAppName("UserBasedCollaborativeFilteringDemo")
      .setMaster("local[*]")
      .set("spark.sql.warehouse.dir", "warehouse")
    val sc = new SparkContext(conf)

    // test.data是用户_物品_评分样本，且用户为Int，物品为int型
    val data = sc.textFile("data/UserBasedCollaborativeFilteringDemo_data.txt")
    val parseData: RDD[MatrixEntry] = data.map(_.split(",") match { case Array(user, item, rate) =>
      MatrixEntry(user.toLong - 1, item.toLong - 1, rate.toDouble)
    })
    ...
    //把CoordinateMatrix转换成RowMatrix来计算两个用户之间的cos相似性，且行表示用户，列表示物品
    //RowMatrix的方法，columnSimilarities是计算列与列的相似度，现在是user_item_rating，
    //需要转置(transpose)成item_user_rating,这样才是用户的相似
